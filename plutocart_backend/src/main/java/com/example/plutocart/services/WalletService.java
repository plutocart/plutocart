...
//            UserDetails userDetails = (UserDetails) SecurityContextHolder.getContext().getAuthentication().getPrincipal(); // wait login
        ...
        walletRepository.insertWalletByAccountID(wallet.getNameWallet(), wallet.getBalanceWallet(), accountRepository.findById(1).get().getId(), LocalDateTime.now(), LocalDateTime.now()); // account gues
